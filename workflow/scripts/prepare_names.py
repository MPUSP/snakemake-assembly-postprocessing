...
# read sample sheet
try:
    df_samples = pd.read_csv(sample_sheet, dtype=str)
    sys.stderr.write(f"Read sample sheet from {snakemake.params['sample_sheet']}\n")
except Exception as e:
    raise RuntimeError(
        f"Failed to read sample sheet from {snakemake.params['sample_sheet']}: {e}"
    ) from e
    ...
    df_ref = pd.DataFrame(
        {
            ...
            "sample": [ref_name] if ref_name else [os.path.basename(ref_file)],
        }
    )
    df_samples = pd.concat([df_samples, df_ref], ignore_index=True)

# ntSynt mutates "_" to " "
df_samples["sample"] = df_samples["sample"].apply(
...
try:
    ...
    raise RuntimeError(
        f"Failed to write ntSynt-viz sample name mapping to {outfile}: {e}"
    ) from e
